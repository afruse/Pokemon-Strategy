...
public class HyperBeam extends Attack
{
     ...
     */
    public HyperBeam(BattleOrderActionBlock a, BattleOrderActionBlock b, boolean original)
    {
        GreenfootImage image = new GreenfootImage("AttackSprites/hyperbeam.png");
        this.setImage(image);
        originX = a.getX();
        originY = a.getY();
        targetX = b.getX();
        ...
        this.speed = 6;
        this.getImage().scale(50, 80);
        this.getImage().rotate(270);
        // Pythagorean theorem for distance
        distance = Math.sqrt( (targetY-originY)*(targetY-originY) + (targetX-originX)*(targetX-originX) );
    }
    
    public HyperBeam(BattleOrderActionBlock a, BattleOrderActionBlock b, boolean original, int rotation)
    {
        GreenfootImage image = new GreenfootImage("AttackSprites/hyperbeam.png");
        this.setImage(image);
        originX = a.getX();
        ...
        this.speed = 25;
        this.getImage().scale( Greenfoot.getRandomNumber(20) + 60, Greenfoot.getRandomNumber(20) + 70 );
        this.getImage().rotate(270);
        // Pythagorean theorem for distance
        distance = Math.sqrt( (targetY-originY)*(targetY-originY) + (targetX-originX)*(targetX-originX) );
    }
    ...
    public void act()
    {
        ...
        if (original)
        {
            ...
            if (timer % 1 == 0)
            {
                ...
                HyperBeam beam = new HyperBeam(a, b, false, rotation);
